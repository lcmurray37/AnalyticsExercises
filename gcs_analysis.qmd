...
```{r}
final_do_list$tenth1 <- safe_api_call(do_tenths$tenth1)
```

```{r}
test <- map(do_tenths$tenth1, summary_request)
```
...
```{r}
# contracts with no DOs
usa_do_list <- map(do_tenths, perform_request)
```

### Store Contract data in list

```{r}
usa_do_df <- combine_results(usa_do_list)
```

## Create combined df

```{r}
usaspending_df <- rbind(usa_df, usa_do_df)
```
